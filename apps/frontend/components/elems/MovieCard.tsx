...
export function MovieCard({ movie }: { movie: MovieDTO }) {
	...
	const [likedStyle, setLikedStyle] = useState<string>('white')
	const [liked, setLiked] = useState<boolean>(false)

	useEffect(() => {
        setColorHeart(movie.liked ? 'orange-50' : 'white')
        setLikedStyle(movie.liked ? '' : 'hidden group-hover:block')
	}, [])

	async function likeMovie() {
		try {

			const response = await axios.get(`http://localhost:5001/movies/like/${movie.imdb_code}`, {
			...
			if (response.data === 'Movie liked') {
				...
				setLiked(false)
				setColorHeart('white')
                setLikedStyle('hidden group-hover:block')
			}

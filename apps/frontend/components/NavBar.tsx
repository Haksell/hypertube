...
function ButtonLinkNavBar({
    ...
    let styleInit: string = `text-gray-300 hover:bg-gray-700 hover:text-white rounded-md px-3 py-2 text-sm font-medium ${block && 'block'
        }`;
    const [style, setStyle] = useState<string>(styleInit);
    useEffect(() => {
        if (page.match(currLink)) {
            setStyle(`bg-gray-900 text-white rounded-md px-3 py-2 text-sm font-medium ${block && 'block'
                }`)
        }
        else

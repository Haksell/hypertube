...
function MoviePage() {
    ...
    const IconButton = ({ icon, color, hoverColor, onClick }) => (
        <button className={`mr-10`} onClick={onClick}>
          <svg className={`w-6 h-6 text-slate-200 hover:text-${hoverColor} hover:duration-300 ease-in`} fill="currentColor" viewBox="0 0 24 24">
            {icon}
          </svg>
        </button>
    );
    ...
    const ImageList = ({ title, items }) => (
        <div className='relative pl-2 flex flex-none mr-5 my-12'>
            <div className="absolute px-1 left-0 -top-9 flex flex-row items-center w-full">
                ...
                <hr className="mt-1 ml-2 w-full h-px bg-gray-200 border-0 dark:bg-gray-700"/>
            </div>
